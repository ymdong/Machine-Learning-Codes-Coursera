...
for i=1:m
    Y=zeros(1,num_labels);Y(y(i))=1;
    J=J-Y*log(a3(i,:))'-(1-Y)*log(1-a3(i,:))';
end

J=1/m*J+lambda/(2*m)*(sum(sum((Theta1(:,2:end)).^2))+sum(sum(Theta2(:,2:end).^2)));

Delta_1=zeros(size(Theta1));Delta_2=zeros(size(Theta2));
for i=1:m
    a_1=[1;X(i,:)'];z_2=Theta1*a_1;
    a_2=[1;sigmoid(z_2)];z_3=Theta2*a_2;a_3=sigmoid(z_3);
